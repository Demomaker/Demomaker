<!DOCTYPE HTML>
<!--
	Phantom by HTML5 UP
	html5up.net | @ajlkn
	Free for personal and commercial use under the CCA 3.0 license (html5up.net/license)
-->
<html>

<head>
	<title>Demomaker</title>
	<meta charset="utf-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
	<link rel="stylesheet" href="assets/css/main.css" />
	<link rel="icon"
		href="https://user-images.githubusercontent.com/18319764/74090572-e8c65900-4a7a-11ea-879d-c4dbe442084e.png">
	<noscript>
		<link rel="stylesheet" href="assets/css/noscript.css" /></noscript>
</head>

<body class="is-preload">
	<!-- Wrapper -->
	<div id="wrapper">

		<!-- Header -->
		<header id="header">
			<div class="inner">

				<!-- Logo -->
				<a href="http://social.demomaker.net" class="logo">
					<span class="symbol"><img src="images/Demomakerlogo.svg" alt="" /></span><span
						class="title">Demomaker</span>
				</a>

				<!-- Nav -->
				<nav>
					<ul>
						<li><a href="#menu">Menu</a></li>
					</ul>
				</nav>

			</div>
		</header>

		<!-- Menu -->
		<nav id="menu">
			<h2>Menu</h2>
			<ul>
				<li><a href="http://social.demomaker.net">Home</a></li>
				<li><a href="gaming.php">Gaming Videos</a></li>
				<li><a href="music">Music</a></li>
				<li><a href="computerscience">Computer Science</a></li>
				<li><a href="literature">Literature</a></li>
			</ul>
		</nav>

		<!-- Main -->
		<div id="main">
			<div class="inner">
				<header>
					<p>Hello everyone, this is Demomaker...</p>
					<h1>This is Demomaker's Website to put all of the important stuff.<br />
				</header>
				<section class="tiles">
					<article class="style1">
						<span class="image">
							<img src="images/pic01.jpg" alt="" />
						</span>
						<a href="generic.html">
							<h2>Gaming Videos</h2>
							<div class="content">
								<p>View all of the gaming videos I've made throughout the years.</p>
							</div>
						</a>
					</article>
					<article class="style2">
						<span class="image">
							<img src="images/pic02.jpg" alt="" />
						</span>
						<a href="generic.html">
							<h2>Music</h2>
							<div class="content">
								<p>View all of the music I have produced.</p>
							</div>
						</a>
					</article>
					<article class="style3">
						<span class="image">
							<img src="images/pic03.jpg" alt="" />
						</span>
						<a href="generic.html">
							<h2>Computer Science</h2>
							<div class="content">
								<p>View my Computer Science-related projects.</p>
							</div>
						</a>
					</article>
					<article class="style4">
						<span class="image">
							<img src="images/pic04.jpg" alt="" />
						</span>
						<a href="generic.html">
							<h2>Litterature</h2>
							<div class="content">
								<p>View books that were written by yours truly.</p>
							</div>
						</a>
					</article>
				</section>
			</div>
		</div>

		<!-- Footer -->
		<footer id="footer">
			<div class="inner">
				<section>
					<h2>Follow</h2>
					<ul class="icons">
						<li><a href="https://www.demomaker.net/twitter" class="icon brands style2 fa-twitter"><span
									class="label">Twitter</span></a></li>
						<li><a href="https://www.demomaker.net/youtube" class="icon brands style2 fa-youtube"><span
									class="label">Youtube</span></a></li>
						<li><a href="https://www.demomaker.net/spotify" class="icon brands style2 fa-spotify"><span
									class="label">Spotify</span></a></li>
						<li><a href="https://www.demomaker.net/soundcloud"
								class="icon brands style2 fa-soundcloud"><span class="label">SoundCloud</span></a></li>
						<li><a href="https://www.demomaker.net/github" class="icon brands style2 fa-github"><span
									class="label">GitHub</span></a></li>
					</ul>
				</section>
				<ul class="copyright">
					<li>&copy; Demomaker. All rights reserved</li>
					<li>Design: <a href="http://html5up.net">HTML5 UP</a></li>
				</ul>
			</div>
		</footer>

	</div>

	<!-- Scripts -->
	<script src="assets/js/jquery.min.js"></script>
	<script src="assets/js/browser.min.js"></script>
	<script src="assets/js/breakpoints.min.js"></script>
	<script src="assets/js/util.js"></script>
	<script src="assets/js/main.js"></script>

</body>

</html>